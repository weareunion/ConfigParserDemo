...
class ParseError extends ConfigError{
    constructor(lineIndex:number, line:string, error?:string) {
        super(`Error on line ${lineIndex} : "${line}"${(error ?? "")}`);
...
class DuplicateIndex extends ConfigError{
    constructor(line:number, indexName:string) {
...
class ConfigurationFile {
    /**
     * Contains the raw configuration text for
     * later reference if needed
     * @private
     */
    private _configurationTextRaw;
    /**
     * Contains a map of the configuration data,
     * with the map key = to the configuration key,
     * and the map value = the configuration value after
     * processing and setting the datatype.
     *
     * Map<string, Boolean|String|Number>
     * @private
     */
    private _map;
     ...
     * processed or not
     */
    constructor(configurationText: string, autoProcess: boolean = true) {
        // Set the config text to the field
        this._configurationTextRaw = configurationText

        // Instantiate Map
        this._map = new Map<string, Boolean|String|Number>;

        // If requested, process the data in
        if (autoProcess) this.process()
    }
    ...
    process(){
        ...
        line_split.forEach((line:string, index:number) => {
            ...
            if (splitLine.length == 2) splitLine[1] = splitLine[1].trim();
     ...
     */
    toOBJ(){
        let Obj:{[key:string]:[value:String|Number|Boolean]} = {};

        // Iterate through map and add to object
        this._map.forEach((value:any, key) => {
            Obj[key] = value
        })
     ...
     * @private
     */
    private processDataType(data:string){
        // Trim data
        data = data.trim()
        ...
        // one of the lists
        if (this._booleanEquivalents.true.includes(data.toLowerCase())) return true;
        if (this._booleanEquivalents.false.includes(data.toLowerCase())) return false;
     ...
     * @private
     */
    private static splitByLines(input:string){
